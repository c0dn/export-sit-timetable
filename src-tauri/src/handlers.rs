...
#[tauri::command]
pub async fn export_to_ics(state: State<'_, Mutex<AppState>>, path: String) -> Result<(), String> {
...
pub async fn is_update_available(
    ...
    if version != latest_version {
        app.dialog()
            ...
            .title("New Version")
            .buttons(MessageDialogButtons::OkCustom("Download".to_string()))
            .show(move |result| {
                if result {
                    let opener = app.opener();
                    let _ = opener.open_url(
                        "https://github.com/c0dn/export-sit-timetable/releases",
                        None::<&str>,
                    );
                }
            });
